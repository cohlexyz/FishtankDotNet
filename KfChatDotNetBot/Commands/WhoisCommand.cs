...
public class WhoisCommand : ICommand
{
    ...
    public async Task RunCommand(ChatBot botInstance, MessageModel message, UserDbModel user, GroupCollection arguments, CancellationToken ctx)
    {
        ...
        if (queryUser != null)
        {
            await botInstance.SendChatMessageAsync($"{user.FormatUsername()}, {queryUser.KfUsername}'s ID is {queryUser.KfId}", true, whisperTo: user.KfUsername);
            return;
        }
        ...
        await botInstance.SendChatMessageAsync($"{user.FormatUsername()}, my guess is you're looking for {queryUser!.KfUsername} whose ID is {queryUser.KfId}", true, whisperTo: user.KfUsername);
    }
}
